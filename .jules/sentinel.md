...
## 2024-04-26 - [Mass Assignment in Contact Form]
**Vulnerability:** The POST `/api/contact` endpoint directly passed `req.body` to the Mongoose `Contact` model.
**Learning:** Even for simple forms like contact messages, directly passing the request body allows users to modify internal fields like `status` or `createdAt` which shouldn't be user-controllable.
**Prevention:** Always explicitly destructure inputs before passing them to the database model to only allow expected fields.
