...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          // ⚡ Bolt: Use requestAnimationFrame to throttle scroll events and prevent layout thrashing
          const isScrolled = window.scrollY > 20;
          setScrolled(isScrolled);

          const winScroll = document.body.scrollTop || document.documentElement.scrollTop;
          const height = document.documentElement.scrollHeight - document.documentElement.clientHeight;
          // ⚡ Bolt: Update scroll progress state only once per animation frame
    ...
    // ⚡ Bolt: Add passive: true to improve scrolling performance by not blocking the main thread
