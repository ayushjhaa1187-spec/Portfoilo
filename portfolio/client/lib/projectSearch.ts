...
export function searchProjects(projects: Project[], opts: SearchOptions): Project[] {
    ...
    results = results
      .map((project): ScoredProject => ({
        ...
        score: tokens.reduce((acc, token) => (
          ...
          ((project.tags ?? []).some((tag) => tag.toLowerCase().includes(token)) ? 1 : 0)
        ), 0)
      }))
  ...
  switch (opts.sortBy) {
    ...
    case 'accuracy':
      // Sort by first metric value if available, otherwise by featured
      return [...results].sort((a, b) => {
        const aVal = parseFloat((a.metrics?.[0]?.value ?? '0').replace(/[^\d.]/g, ''));
        const bVal = parseFloat((b.metrics?.[0]?.value ?? '0').replace(/[^\d.]/g, ''));
        return bVal - aVal;
      });
    case 'complexity':
